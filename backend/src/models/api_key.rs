...
fn decrypt_key(encrypted: &str) -> Result<String, String> {
    ...
    if combined.len() < 12 { return Err("invalid encrypted key".to_string()); }
...
pub struct ApiKey {
    pub id: i64, pub org_id: i64, pub user_id: i64, pub name: String,
    #[serde(skip_serializing)] pub key_hash: String,
    #[serde(skip_serializing)] pub key_encrypted: String,
    pub key_prefix: String, pub scopes: String,
    pub last_used_at: Option<chrono::DateTime<chrono::Utc>>, pub created_at: chrono::DateTime<chrono::Utc>, pub revoked_at: Option<chrono::DateTime<chrono::Utc>>,
}

#[derive(Debug, Serialize)]
pub struct ApiKeyCreated {
    pub id: i64, pub name: String, pub key: String,
    pub key_prefix: String, pub scopes: String, pub created_at: chrono::DateTime<chrono::Utc>,
}

#[derive(Debug, Serialize)]
pub struct ApiKeyRow {
    pub id: i64, pub name: String, pub key_prefix: String, pub scopes: String,
    pub last_used_at: Option<chrono::DateTime<chrono::Utc>>, pub status: String, pub created_at: chrono::DateTime<chrono::Utc>,
}
...
impl From<ApiKey> for ApiKeyRow {
    fn from(k: ApiKey) -> Self {
        Self {
            ...
            status: if k.revoked_at.is_some() { "revoked" } else { "active" }.into(),
...
impl ApiKey {
    ...
    pub fn reveal(&self) -> Result<String, String> { decrypt_key(&self.key_encrypted) }

    pub async fn create(
        pool: &MySqlPool, org_id: i64, user_id: i64, name: &str, scopes: &str,
        ...
        let created = ApiKeyCreated { id: row.id, name: row.name, key: full_key.clone(),
            key_prefix: row.key_prefix, scopes: row.scopes, created_at: row.created_at };
        Ok((created, full_key))
    }
    ...
    pub async fn find_by_id(pool: &MySqlPool, id: i64, org_id: i64) -> Result<Option<ApiKey>, sqlx::Error> {
        sqlx::query_as::<_, ApiKey>("SELECT * FROM api_keys WHERE id = ? AND org_id = ? AND revoked_at IS NULL")
            .bind(id).bind(org_id).fetch_optional(pool).await
    }

    pub async fn find_by_hash(pool: &MySqlPool, key_hash: &str) -> Result<Option<ApiKey>, sqlx::Error> {
