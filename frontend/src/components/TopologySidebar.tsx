  ...
  return (
    <div className="w-60 shrink-0 space-y-4">
      ...
      <div className="flex items-center justify-between px-1">
        <h3 className="text-sm font-semibold text-fg-primary tracking-tight">Topology Controls</h3>
        <span className="text-[11px] font-mono text-fg-tertiary">{nodes.length} nodes</span>
      </div>
      ...
      {/* Layout controls */}
      <div>
        <h4 className="text-xs font-semibold text-fg-tertiary uppercase tracking-wider mb-2 px-1">Layout</h4>
        <Radio.Group
          value={activeLayout}
      ...
      {/* Sizing controls */}
      <div>
        <h4 className="text-xs font-semibold text-fg-tertiary uppercase tracking-wider mb-2 px-1">Node size by</h4>
        <Radio.Group
          value={activeSizing}
      ...
      <div>
        <button
          ...
          className="w-full flex items-center justify-between px-1 py-1 text-xs font-semibold text-fg-tertiary uppercase tracking-wider hover:text-fg-secondary transition-colors"
        >
          Node Legend
          <span className="text-fg-tertiary font-mono text-[11px]">{sortedNodes.length}</span>
        </button>
        {expanded && (
          <div className="max-h-[300px] overflow-y-auto overscroll-contain space-y-px mt-1">
            {sortedNodes.length === 0 ? (
              <p className="text-[11px] text-fg-tertiary px-2.5 py-2 italic">No nodes match search</p>
            ) : (
